...
3. `WalletPage` function is not export so it will not be able to be used anywhere
...
4. `getPriority` has some anti-patterns and inefficiency issue
...
5. Incorrect filtering logic, condition, referencing undefined variables
...
6. No type for `blockchain` in `WalletBalance`
  ...
  7. `Prices` dependency in useMemo is not used
...
8. Using `.toFixed()` without specifying decimals
...
9. Mapping `formattedBalances` is unused
...
10. Have 2 map operations for `sortedBalances` array is not neccessary
...
11. Rendering rows using index as key
